...
- `sanctum/*.md` is markdown on the filesystem; SQL-style triggers
  don't apply. The CM check (`ai-meta.sh` #6) is the equivalent
...
- **Not unique to Polaris.** ADR/RFC processes are also
  ...
  The principle is being named here because Polaris applies it four
...
- **Structural-invariant test** at CI time (added v8.50): scans
  every `.sql` file under `polaris_sql/` for the substrings
  `ON DELETE CASCADE` and `ON UPDATE CASCADE`. Fails if any match.
  See `test_no_fk_cascade_in_polaris_sql` in
  `polaris_web/test_structural_invariants.py`.
- **No allowlist mechanism.** If a future schema genuinely needs
  CASCADE semantics (very unlikely — Polaris's identity model
  rejects it constitutionally), the right path is a Sanctum-class
...
- Not applied to cognitive-layer artifacts. The Sanctum filesystem
  AoR instance (`sanctum/*.md`) has no FK semantics; the rule is
...
- `polaris_sql/06_triggers.sql` — `reject_audit_modification`,
  ...
  layer.
- `meta/sanctum-protocol.md` — Sanctum sessions as the
  cognitive-layer instance.
- `scripts/ai-meta.sh` — CM check #6 enforcement for the Sanctum
  instance.
- `DEVNOTES/concurrency.md` — adjacent principle: per-entity
  advisory locks are the concurrency complement to audit-of-record.
- v8.19 self-audit (in chat / journal entry 33) — the audit that
