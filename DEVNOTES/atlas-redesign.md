...
| 7 | v9.265 | **Trends.** An hour×weekday temporal-rhythm heatmap (atlas_heatmap, 168 cells) + a stack-by-dimension composition-over-time chart (atlas_series_stacked, top-K + Other), both bounded (C8), C6-safe, partition-pruned, hand-rolled SVG, driven by the global filter and verified in a real browser by the UI drill. (Brush/zoom, crosshair, compare-to-previous carry into a follow-up.) | done |
| 8 | TBD | **Investigate (entity workspace).** Search any entity (subject / agency / token) → linked timeline + geography + relations + activity; warrant- and agency-audit as a real entity view | planned |
| 9 | TBD | **Alerts + saved views + final production pass.** Surfaced needs-attention queries; saveable filter/view state; accessibility, responsive and 2M+ performance pass | planned |
