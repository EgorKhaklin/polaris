...
\usepackage[
  ...
  pdftitle={Polaris - Identity Token System},
]{hyperref}
...
% Allow more flexible line breaking for paragraphs heavy with long identifiers.
\sloppy
...
% Image is square (1:1).
\includegraphics[height=8cm]{assets/polaris_logo_clean.png}
...
The appendices are not afterthoughts. They are the dimensions in which the body's claims are tested. Appendix~\ref{app:state-machine} formalizes the lifecycle invariants that the schema's \texttt{CHECK} constraints alone cannot express. Appendix~\ref{app:indexing} addresses the query-plan and partitioning concerns that determine whether the schema can carry national-scale verification volume. Appendix~\ref{app:comparison} situates the design against Estonia's eID, Aadhaar, mDL, EU eIDAS 2.0, and the W3C verifiable-credentials stack, locating the distinguishing feature (\texttt{disclosure\_level} as a schema-level mechanism) against five existing approaches. Appendix~\ref{app:threat-model} runs the schema through STRIDE and names the three threats the database layer cannot mitigate. Appendix~\ref{app:substrate} formalizes the deeper architectural argument: cryptographic primitives are the substrate of digital sovereignty, and every higher-level property is derivative of them. Appendix~\ref{app:beyond-biometric} sketches the two horizons beyond fingerprint-, face-, and iris-binding (genomic and quantum-observer) and the schema extensions each would require.
...
\rowcolors{2}{tabstripe}{white}
\begin{tabularx}{\textwidth}{>{\bfseries}p{1.55in} >{\RaggedRight\arraybackslash}X}
\toprule
\rowcolor{tabheader}
...
\renewcommand{\arraystretch}{1.1}
\rowcolors{2}{tabstripe}{white}
\begin{tabularx}{\textwidth}{>{\RaggedRight\arraybackslash}p{1.95in} >{\centering\arraybackslash}p{0.75in} >{\RaggedRight\arraybackslash}X}
\toprule
\rowcolor{tabheader}
...
\textbf{Estonia X-Road and e-Residency.} Estonia operates the most mature national digital-identity infrastructure in production. Every Estonian resident has a state-issued chip card with two PKI key pairs (authentication and signing) under a state-operated certificate authority. X-Road is the federated data-exchange backbone that sits behind the card, allowing agencies to verify identity and exchange data without centralizing it. The architecture has been operating since 2002 and is the closest live analogue to the present design, but Estonia's PKI is not post-quantum.
...
\midrule
Estonia X-Road / eID  & Federated state CA & Optional, on-device & OCSP / CRL & Classical (RSA, ECDSA) \\
