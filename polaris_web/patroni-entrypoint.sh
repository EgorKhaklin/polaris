...
# script as the command and `user: postgres`. The image ENTRYPOINT still runs
# first (pg-entrypoint.sh renders the pgBackRest repo fragment, then the stock
# docker-entrypoint.sh execs a command that is not `postgres` untouched), so
# everything the single-node stack renders at start is rendered here too.
#
