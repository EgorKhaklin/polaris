...
-- ============================================================================
-- S. PHEROMONE ROTATION FRAMEWORK (v9.07 / D5-impl + v9.10 / S2 Layer-1)
-- ============================================================================
-- Closes the structural-enforcement gap surfaced by the v9.10 / S2 Sanctum
-- (cognitive-layer-ratio Position C requires v9.10 to ship a Layer-1
-- candidate). The v9.07 framework already had end-to-end shell-script
-- drill but no SQL-level invariants pinning the trigger semantics. The
-- 10 tests below mirror the v8.87 audit-log archive+purge framework
-- shape: trigger-rejection paths + GUC carve-out + checkpoint
-- append-only + CHECK constraints + admin-role validation. Each test
-- is hermetic (no row left behind; rollback or compensating action on
-- exit) so 08_tests.sql remains idempotent under repeated load.
--
-- Gating: the framework lives in a v9.07 migration, not in the base
-- schema load, so on a fresh dropdb+createdb+00_load_all the
-- LifecyclePheromoneCheckpoint table won't exist yet. Each S.* test
-- checks for the framework's presence; tests are recorded as PASS
-- with a "skipped (migration not applied)" detail when absent. After
-- `polaris-migrate.sh --up` they run in full.

-- S.0: framework presence detector (sets a TEMP setting other tests read).
DO $$
DECLARE v_has_table BOOLEAN; v_has_proc BOOLEAN;
BEGIN
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
         WHERE table_name = 'lifecyclepheromonecheckpoint'
    ) INTO v_has_table;
    SELECT EXISTS (
        SELECT 1 FROM pg_proc
         WHERE proname = 'uc_pheromone_archive_purge'
    ) INTO v_has_proc;
    PERFORM set_config(
        'polaris.test_pheromone_framework_present',
        CASE WHEN v_has_table AND v_has_proc THEN 'TRUE' ELSE 'FALSE' END,
        false
    );
END $$;

-- S.1: trg_pheromone_append_only rejects raw DELETE on an existing row.
--      INSERT first (allowed; trigger only fires on UPDATE/DELETE) then
--      attempt the DELETE without GUC. Trigger must reject; cleanup via
--      the carve-out so the row doesn't pollute later tests.
DO $$
DECLARE v_marker_id BIGINT;
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.1: raw DELETE on Pheromone rejected',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    INSERT INTO Pheromone (
        deposited_at, deposited_by, intensity, node_id, kind,
        evidence, seed
    ) VALUES (
        now() - INTERVAL '1 hour', 'ant_test_s1_marker', 0.5,
        'test:s1', 'info', '{}'::jsonb, 11111
    ) RETURNING pheromone_id INTO v_marker_id;

    BEGIN
        DELETE FROM Pheromone WHERE pheromone_id = v_marker_id;
        PERFORM _record('S.1: raw DELETE on Pheromone rejected',
            FALSE, 'raw DELETE unexpectedly accepted');
    EXCEPTION WHEN insufficient_privilege THEN
        PERFORM _record('S.1: raw DELETE on Pheromone rejected',
            TRUE, NULL);
    END;

    -- Cleanup via carve-out
    PERFORM set_config('polaris.pheromone_purge_in_progress', 'TRUE', true);
    DELETE FROM Pheromone WHERE pheromone_id = v_marker_id;
END $$;

-- S.2: trg_pheromone_append_only rejects raw UPDATE on an existing row
--      (always — no carve-out for UPDATE).
DO $$
DECLARE v_marker_id BIGINT;
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.2: raw UPDATE on Pheromone rejected',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    INSERT INTO Pheromone (
        ...
        now() - INTERVAL '1 hour', 'ant_test_s2_marker', 0.5,
        'test:s2', 'info', '{}'::jsonb, 22222
    ) RETURNING pheromone_id INTO v_marker_id;

    BEGIN
        UPDATE Pheromone SET intensity = 0.6 WHERE pheromone_id = v_marker_id;
        PERFORM _record('S.2: raw UPDATE on Pheromone rejected',
            FALSE, 'raw UPDATE unexpectedly accepted');
    EXCEPTION WHEN insufficient_privilege THEN
        PERFORM _record('S.2: raw UPDATE on Pheromone rejected',
            TRUE, NULL);
    END;
...
-- S.3: SET LOCAL polaris.pheromone_purge_in_progress='TRUE' opens the
--      DELETE carve-out (without actually deleting any seed data).
DO $$
DECLARE v_marker_id BIGINT; v_pre INTEGER; v_post INTEGER;
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.3: GUC carve-out allows DELETE',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    -- Create a disposable marker row in its own subtxn so the test
    -- doesn't pollute seed data.
    INSERT INTO Pheromone (
        deposited_at, deposited_by, intensity, node_id, kind,
        evidence, seed
    ) VALUES (
        now() - INTERVAL '1 hour', 'ant_test_s3_marker', 0.5,
        'test:s3', 'info', '{}'::jsonb, 12345
    ) RETURNING pheromone_id INTO v_marker_id;

    SELECT count(*) INTO v_pre FROM Pheromone WHERE pheromone_id = v_marker_id;

    BEGIN
        PERFORM set_config('polaris.pheromone_purge_in_progress', 'TRUE', true);
        DELETE FROM Pheromone WHERE pheromone_id = v_marker_id;
        SELECT count(*) INTO v_post FROM Pheromone WHERE pheromone_id = v_marker_id;
        PERFORM _record('S.3: GUC carve-out allows DELETE',
            v_pre = 1 AND v_post = 0,
            format('pre=%s post=%s', v_pre, v_post));
    EXCEPTION WHEN insufficient_privilege THEN
        -- If carve-out failed, marker row still exists; clean up directly
        -- isn't possible (no carve-out), so leave it but record failure.
        PERFORM _record('S.3: GUC carve-out allows DELETE',
            FALSE, 'DELETE rejected even with GUC=TRUE');
    END;
END $$;

-- S.4: GUC carve-out evaporates after subtxn boundary (SET LOCAL semantics).
--      A second DELETE in a fresh subtxn (no SET LOCAL) must be rejected.
DO $$
DECLARE v_marker_id BIGINT;
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.4: carve-out evaporates without SET LOCAL',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    INSERT INTO Pheromone (
        ...
        now() - INTERVAL '1 hour', 'ant_test_s4_marker', 0.5,
        'test:s4', 'info', '{}'::jsonb, 23456
    ) RETURNING pheromone_id INTO v_marker_id;

    BEGIN
        -- No SET LOCAL this time; fresh DO-block has its own setting scope.
        DELETE FROM Pheromone WHERE pheromone_id = v_marker_id;
        PERFORM _record('S.4: carve-out evaporates without SET LOCAL',
            FALSE, 'DELETE accepted without GUC=TRUE');
    EXCEPTION WHEN insufficient_privilege THEN
        PERFORM _record('S.4: carve-out evaporates without SET LOCAL',
            TRUE, NULL);
    END;

    -- Cleanup: re-open carve-out, delete marker.
    PERFORM set_config('polaris.pheromone_purge_in_progress', 'TRUE', true);
    DELETE FROM Pheromone WHERE pheromone_id = v_marker_id;
END $$;

-- S.5: LifecyclePheromoneCheckpoint is strictly append-only (no GUC carve-out
--      at the checkpoint layer per G32). DELETE must reject even with the
--      Pheromone-purge GUC active. INSERT a probe row first, then attempt
--      DELETE — the row stays as audit-of-record (cannot be cleaned up
--      since the table is strictly append-only).
DO $$
DECLARE v_probe_id BIGINT; v_admin_id INTEGER;
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.5: LifecyclePheromoneCheckpoint append-only DELETE',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    SELECT user_id INTO v_admin_id FROM AppUser WHERE role = 'admin' LIMIT 1;
    -- Probe directly into the table (raw INSERT is allowed — the trigger
    ...
    INSERT INTO LifecyclePheromoneCheckpoint (
        ...
        now() - INTERVAL '1 hour',
        '/tmp/test-s5-probe.tar.gz',
        repeat('e', 64),
        COALESCE(v_admin_id, 1),
        0
    ) RETURNING checkpoint_id INTO v_probe_id;

    BEGIN
        PERFORM set_config('polaris.pheromone_purge_in_progress', 'TRUE', true);
        DELETE FROM LifecyclePheromoneCheckpoint WHERE checkpoint_id = v_probe_id;
        PERFORM _record('S.5: LifecyclePheromoneCheckpoint append-only DELETE',
            FALSE, 'DELETE on checkpoint unexpectedly accepted');
    EXCEPTION WHEN insufficient_privilege THEN
        PERFORM _record('S.5: LifecyclePheromoneCheckpoint append-only DELETE',
            TRUE, NULL);
    END;
    -- Probe row stays. That's the point of strictly-append-only.
END $$;
...
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.6: archive_sha256 hex CHECK rejects malformed',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    BEGIN
        INSERT INTO LifecyclePheromoneCheckpoint (
            ...
            now() - INTERVAL '1 day', '/tmp/notreal',
            'not_a_real_sha256_hash_only_29_ch',  -- not 64 hex chars
            1, 0
        );
        PERFORM _record('S.6: archive_sha256 hex CHECK rejects malformed',
            FALSE, 'malformed sha256 unexpectedly accepted');
    EXCEPTION WHEN check_violation THEN
        PERFORM _record('S.6: archive_sha256 hex CHECK rejects malformed',
            TRUE, NULL);
    END;
...
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.7: cutoff_in_past CHECK rejects future ts',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    BEGIN
        INSERT INTO LifecyclePheromoneCheckpoint (
            ...
            '/tmp/notreal',
            repeat('a', 64),  -- valid hex shape
            1, 0
        );
        PERFORM _record('S.7: cutoff_in_past CHECK rejects future ts',
            FALSE, 'future cutoff unexpectedly accepted');
    EXCEPTION WHEN check_violation THEN
        PERFORM _record('S.7: cutoff_in_past CHECK rejects future ts',
            TRUE, NULL);
    END;
...
-- S.8: pheromone_rows_purged_nonneg CHECK rejects negative rows_purged.
DO $$
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.8: rows_purged_nonneg CHECK rejects negative',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    BEGIN
        INSERT INTO LifecyclePheromoneCheckpoint (
            ...
            now() - INTERVAL '1 day', '/tmp/notreal',
            repeat('b', 64), 1, -5  -- negative
        );
        PERFORM _record('S.8: rows_purged_nonneg CHECK rejects negative',
            FALSE, 'negative rows_purged unexpectedly accepted');
    EXCEPTION WHEN check_violation THEN
        PERFORM _record('S.8: rows_purged_nonneg CHECK rejects negative',
            TRUE, NULL);
    END;
...
-- S.9: uc_pheromone_archive_purge() rejects non-admin actor.
DO $$
DECLARE v_non_admin_id INTEGER;
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.9: uc_pheromone_archive_purge admin-role gate',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    -- Find a non-admin user; if none exist, record N/A as PASS
    -- (procedure still has the role gate compiled into it — covered
    --  structurally by ai-grep checks against 05_procedures.sql).
    SELECT user_id INTO v_non_admin_id
      FROM AppUser WHERE role <> 'admin' LIMIT 1;
    IF v_non_admin_id IS NULL THEN
        PERFORM _record('S.9: uc_pheromone_archive_purge admin-role gate',
            TRUE, 'no non-admin in seed; skipped');
        RETURN;
    END IF;
    ...
    BEGIN
        ...
        PERFORM _record('S.9: uc_pheromone_archive_purge admin-role gate',
            FALSE, 'non-admin actor unexpectedly accepted');
    EXCEPTION WHEN insufficient_privilege THEN
        PERFORM _record('S.9: uc_pheromone_archive_purge admin-role gate',
            TRUE, NULL);
    END;
...
-- S.10: uc_pheromone_archive_purge() rejects non-existent actor_user_id.
DO $$
BEGIN
    IF current_setting('polaris.test_pheromone_framework_present', true) <> 'TRUE' THEN
        PERFORM _record('S.10: uc_pheromone_archive_purge actor-exists gate',
            TRUE, 'skipped (migration 2026-05-15-001 not applied)');
        RETURN;
    END IF;
    BEGIN
        CALL uc_pheromone_archive_purge(
            ...
            '/tmp/notreal',
            repeat('d', 64),
            -99999  -- definitely doesn't exist
        );
        PERFORM _record('S.10: uc_pheromone_archive_purge actor-exists gate',
            FALSE, 'non-existent actor unexpectedly accepted');
