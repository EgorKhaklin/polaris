...
inspiration is traced in `meta/lineage.md`. The machine-checkable
enforcement of the constraints themselves lives in `polaris_checks/`.
...
Three pillars by four tiers, plus one APEX and one MANIFEST:
...
- **APEX** is the architectural intent, not a code-level check: the
  principle from which the other constraints derive meaning.
...
**Position:** Expand-pillar top. The expansive principle: the
catalog of trusted primitives before it gets categorized.
...
algorithms is balanced by contraction of leakage; both are
required for the system to remain coherent.
...
discipline that gives the catalog form.
...
denied entry. This is openness with a limit, not unconditional
permissiveness.
...
**Position:** Balance-pillar middle. The reconciling center, where
the four nodes above (APEX, EXPAND·1, CONTRACT·1,
...
wants." The client's representation is not authoritative; the
server's truth is the truth.
...
**Position:** Manifest, the system as it actually runs, not as the
upper abstractions describe it. The empirical layer.
...
The constraints themselves are enforced at the database level
(triggers, partial unique indexes, CHECK constraints) and checked
in CI by `polaris_checks/` (one `check_*(repo_root)` function per
constraint, run via `python3 -m polaris_checks.run`). This document
is the mapping that explains WHY each constraint sits where it does
and what its neighbors and complement are. Consult it when you're
about to change a constraint and want to know what else might need
to move.

The mapping and `MISSION.md` must stay in sync: if you change one,
change the other. Adding C11 requires updating this document.
...
4. Add the corresponding `check_*` to `polaris_checks/` so the new
   constraint is enforced, not just documented.
5. The lattice mapping is now the test that the addition was
