...
| [>] P0.7 | ZK production profile | M | med | - | Tree depth parameterized (v9.169): runtime `POLARIS_ZK_TREE_DEPTH`, default 14, both prover and Python second witness read it (parity preserved, 31/31); prove/verify/size benchmarked across depths 10-24 and the zk-soundness ledger updated with measured numbers (verify + size constant, prove superlinear via full-tree reconstruction). Plonky2 1.x EVALUATED clean (v9.169): builds, bit-identical roots, two-witness differential passes 18/18 against the 1.x binary; the actual bump ships separately as v9.170 to keep the dependency major a coherent change. Row closes when the bump lands |
