...
| [>] P2.9 | 10M-profile load certification | L | med | P2.1-P2.7 | The published harness drives the planning targets above against the HA topology during a rolling deploy and one induced failover; the numbers are committed. The SINGLE-NODE half is done (v9.256): the P2.14 harness drives the nation at scale and commits the numbers with invariants certified under load ([docs/reference/BENCHMARK.md](docs/reference/BENCHMARK.md)). Remaining: run the same harness against the HA topology during a rolling deploy + a failover |
