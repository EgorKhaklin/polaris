...
Phase 3 opened 2026-05-14 (Position A: Wave-1 autonomous-eligible 5 items in one ship). Wave 1 shipped as v9.01 same day.
...
timestamped tarball with `pg_dump` (custom output) plus the operator
secrets and state, and a `MANIFEST.json` with SHA-256 hashes.
